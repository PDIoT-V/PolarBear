package pdiot.v.polarbear.utils
 ...
 * this information
 */
class RESpeckPacketHandler(private val speckService: BluetoothSpeckService) {
        ...
        for ((_, acc, gyro, _, highFrequency) in r.batchData) {
            ...
            if (currentProcessedMinute != lastProcessedMinute) {
                ...
                if (minuteFrequencies.size < Constants.MINUTES_FOR_MEDIAN_CALC) {
                    ...
                    minuteRespeckFrequencies.sort()
                    val medianRespeckFrequency: Float = if (minuteRespeckFrequencies.size % 2 == 0) {
...
//            }
    ...
    private fun calculateRespeckSamplingFrequency(): Float {
        val numFreq = frequencyTimestampsRespeck.size
        if (numFreq <= 1) {
            return 0f
        }
        val firstTs = frequencyTimestampsRespeck[0]
        val lastTs = frequencyTimestampsRespeck[numFreq - 1]
        val samplingFrequencyRespeck = numFreq * 1f / (lastTs - firstTs) * 1000f
    ...
    companion object {
        ...
        private val storedQueue: Queue<RESpeckStoredSample> = LinkedList()
